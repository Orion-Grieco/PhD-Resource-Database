        ...
        try:
            async with rx.asession() as session:
                result = await session.execute(
                    text(
                        "SELECT id, university_name, program_name, department, degree_type, research_area, strftime('%Y-%m-%d', application_deadline) as application_deadline, tuition_in_state, tuition_out_of_state, url, description FROM programs ORDER BY university_name;"
                    )
                )
